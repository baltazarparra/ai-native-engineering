...
Modelos são treinados em volumes enormes de texto: livros, artigos, código, páginas web. O modelo aprende padrões estatísticos — ele calcula qual token tem mais probabilidade de vir depois do anterior. Isso explica por que modelos geram texto que parece correto mas às vezes é plausível e errado: eles otimizam pra "provável", não pra "verdadeiro".
...
Modelos não sabem o que não sabem — geram respostas mesmo sem informação suficiente. Isso se chama alucinação. Modelos também têm corte de conhecimento: se você pergunta sobre algo lançado ontem, o modelo pode inventar uma resposta baseada em padrões, não em fatos.
...
**Tarefa 3: Implementar o código**. O dev usa Claude Sonnet via Claude Code. Rápido pra iteração e forte em código — não precisa do Opus pra cada função individual.
