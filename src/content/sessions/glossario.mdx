...
**Riscos concretos que aparecem em produção:**

- **RLS ausente:** qualquer usuário pode ler ou alterar dados de outro.
- **Secrets no client:** chaves de API ou tokens visíveis no navegador.
- **Validação só no front:** o backend aceita qualquer payload malicioso.
- **Permissões largas:** o app faz mais do que deveria, sem controle fino.
- **Falta de rollback:** quando quebra, não tem como voltar atrás com segurança.
- **Ausência de logs:** ninguém sabe o que aconteceu quando algo falha.

Gerar algo em 30 minutos não é a parte difícil. Manter amanhã é.
...
- Preview não é produção. Um protótipo pode virar produto, mas só depois de spec, validação, segurança e ownership.
