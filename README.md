...
- **Keep commits reviewable:** non-editorial changes (code, layouts, components, styles, config, and data outside `src/content/**`) should stay under **400 changed lines** per commit (insertions + deletions). Editorial files under `src/content/**` are exempt. See [AGENTS.md](AGENTS.md) → Commit Discipline for the full policy.
